...
class SpatialDatabase {
  calculateDistanceKm(lat1, lon1, lat2, lon2) {
    ...
    const a =
      ...
      Math.cos(lat1 * (Math.PI / 180)) *
        ...
        Math.sin(dLon / 2) ** 2;
    return Number(
      (radius * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a))).toFixed(2),
    );
  }
  ...
  async getBusinesses() {
    try {
      ...
      return require("./data/seedData").businesses;
    }
  }
  ...
  async getDistricts() {
    try {
      ...
      return require("./data/seedData").districts;
    }
  }
  ...
  async getBenchmarks() {
    try {
      ...
      return require("./data/seedData").marketBenchmarks;
    }
  }
  ...
  async getMarketIntelligence(
    ...
    const catBenchmark = benchmarks[category] ||
      benchmarks.other || {
        demandIndex: 75,
        saturation: 40,
        growthRate: 12,
    ...
    const marketGapScore = Math.max(
      10,
      100 -
        (localCompetitionScore * 0.7 + Number(catBenchmark.saturation) * 0.3),
    );
      ...
      potentialScore:
        opportunityScore > 75
          ...
          : opportunityScore > 50
            ? "MODERATE"
            : "CHALLENGING",
      potentialCustomerBase: Math.min(
        45000,
        Math.max(2500, catchmentPopulation),
      ),
      ...
      dataSources:
        dataMode === "live"
          ? [
              ...
              {
                name: "PostgreSQL district and benchmark data",
                type: "database",
              },
            ]
          : [
              {
                name: "PostgreSQL businesses, districts, and benchmarks",
                type: "database",
              },
            ],
    };
    ...
    await pool.query(
      ...
      [
        ...
        assessment.opportunityScore ||
          assessment.marketData?.opportunityScore ||
          null,
  ...
  async getAdminStats() {
    ...
    assessments.forEach((assessment) => {
      ...
      sumFeas +=
        Number(assessment.feasibilityScore || assessment.overallScore) || 78;
      sumOutlay +=
        Number(assessment.projectCost || assessment.expectedInvestment) ||
        140000;
      const category = assessment.businessCategory || "other";
      const scheme =
        assessment.targetScheme ||
        assessment.matchedSchemes?.[0]?.name ||
        "PMEGP";
    ...
    return {
      ...
      categoryBreakdown: Object.entries(catMap).map(([category, count]) => ({
        category,
        count,
      })),
      schemeDistribution: Object.entries(schemeMap).map(([scheme, count]) => ({
        scheme,
        count,
      })),
    };
